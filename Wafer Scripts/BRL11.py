...
# phase matched. Created to produce BRL11 designed by Meng Iu.            #
# Note this wafer was temporarily called BA3 during design.               #
#                                                                         #
# Left in the model to match Meng:                                        #
# -two n-side Bragg stacks closest to core use p-side Bragg materials     #
# -adjusts refractive index in 28% barriers and QWs due to carrier        #
#  injection (but uses wrong thickness to calculate this)                 #
# -uses new refractive index model except for barriers/substrate          #
# -uses different refractive index model for QW at pump and signal/idler  #
#                                                                         #
...
lambda_p  = 783e-9                     #Pump (BRW) wavelength
...
material_2_n = "AlGaAs_{0.6}"                  #n side Bragg stack material a (next to core)
material_2_p = "AlGaAs_{0.7}"                  #n side Bragg stack material a (next to core)
material_matching = "AlGaAs_{0.2}"             #Matching Layer
material_A_b = "AlGaAs_{0.65}"                 #Bottom A Layer
material_A_b_undoped = "AlGaAs_{0.6}"          #Bottom A Layer - undoped section
material_A_t = "AlGaAs_{0.7}"                  #Top A Layer
...
d_A_b = 115e-9             #Bottom A Layer (doped section) thickness
...
d_A_t = 140e-9             #Top A Layer (doped section) thickness
...
materials = [material_substrate,                                         # Substrate
			 *[material_1, material_2_n]*(N_b-2),                        # Bragg stack part 1
			 *[material_1, material_2_p]*2,                              # Bragg stack part 2
			 material_matching,                                          # Matching Layer
			 material_A_b, material_A_b_undoped,                         # A Layer (doped and undoped)
			 *[material_barrier_2, material_barrier_1, material_qw, material_barrier_1]*N_QW, material_barrier_2,  # Barrier/QW structure
			 material_A_t, material_A_t,                                 # A Layer (doped and undoped)
			 material_matching,                                          # Matching Layer
			 *[material_2_p, material_1]*N_t,                            # Bragg stack
...
Nc = Jth*tau/((N_QW*d_QW+(N_QW+1)*d_b1_internal)*100*e) #Injected carriers at threshold [cm^-3]. Total thickness should be N_QW*d_QW+(N_QW+1)*d_b1_internal+d_b1_external*2, but left as is to match Meng
d_n = [-1e-20*Nc if i in [*index_QW,*index_b_1_internal,*index_b_1_external] else 0 for i in range(len(materials))]
