...
# phase matched. Created to reproduce BA0 designed by Meng Iu.            #
#                                                                         #
# Left in the model to match Meng:                                        #
# -adjusts refractive index in barriers and QWs due to carrier injection  #
#  (but only the barriers between the quantum wells, likely an accident)  #
# -still does not match completely, layers are off by 1, 3, and 12 nm but #
#  original design has been lost as this wafer was never meant to be      #
#  produced                                                               #
#                                                                         #
...
lambda_p  = 750e-9                     #Pump (BRW) wavelength
...
material_A1_b = "AlGaAs_{0.6}"                 #Bottom A Layer part 1
material_A2_b = "AlGaAs_{0.5}"                 #Bottom A Layer part 2
...
d_A1_b = 420e-9            #Bottom A Layer (doped section) thickness
d_A2_b = 80e-9             #Bottom A Layer (doped section) thickness
...
q_t = 0                    #Top matching layer mode order
...
layer_names = ["Substrate",                                              # Substrate
			 *["Bragg1", "Bragg2"]*N_b,                                  # Bragg stack
			 ...
			 "ALayer", "ALayer", "ALayer",                               # A Layer (doped and undoped)
...
materials = [material_substrate,                                         # Substrate
			 *[material_1, material_2]*N_b,                              # Bragg stack
			 ...
			 material_A1_b, material_A2_b, material_A2_b,                # A Layer (doped and undoped)
			 *[material_barrier, material_qw]*N_QW, material_barrier,    # Barrier/QW structure
			 material_A_t, material_A_t,                                 # A Layer with Blocking layer
			 material_matching,                                          # Matching Layer
...
doping = [doping_substrate,                                              # Substrate
		  *[doping_n_stack, doping_n_stack]*N_b,                         # Bragg stack
		  ...
		  doping_n_core, doping_n_core, doping_deep_core,                # A Layer (doped and undoped)
...
index_A1_b = A_indices[0]                                                              # Bottom A Layer (doped)
index_A2_b = A_indices[1]                                                              # Bottom A Layer (doped)
...
d_n = [-1e-20*Nc if i in [*index_QW,*index_b[1:-1]] else 0 for i in range(len(materials))] #Only using Nc for QWs and barriers between QWs even though thickness of entire barrier/QW structure was used, but left as is to match Meng
...
for i in range(len(n_eff_p)):
	...
	d_vector[index_A1_b] = d_A1_b
	d_vector[index_A2_b] = d_A2_b
	for j in index_b:
		d_vector[j] = d_b
	for j in index_QW:
		d_vector[j] = d_QW
...
if generate_PICS3D_files:
	...
	PICS3D_file_generation(wafer_name, crosslight_materials, d_vector, doping, d_grading, index_A2_b, index_A_t, index_QW, index_peak, etch_depth)
