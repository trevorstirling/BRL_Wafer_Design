###########################################################################
# BA2 Design Code (called SB2 during growth)                              #
# Solves for matching layer thicknesses to make the following structure   #
# phase matched. Uses Meng Iu's 2022 Phase Matching Model                 #
# Note this wafer was temporarily called BA2i during design.              #
#                                                                         #
...
## Define Variables
wafer_name = 'BA2'
quantum_well_model = "Iu_2022"
AlGaAs_model = "Iu_2022"
#wavelength and mode properties
lambda_p  = 781e-9                     #Pump (BRW) wavelength
...
material_A_max = "AlGaAs_{0.75}"               #A Layer (and grading max)
material_A_min = "AlGaAs_{0.6}"                #A Layer (and grading min)
material_blocking = "AlGaAs_{0.8}"             #Blocking Layer
material_barrier_1 = "AlGaAsP_Barrier_{0.28}"  #Barrier
material_barrier_2 = "AlGaAsP_Barrier_{0.337}" #Barrier
material_qw = "AlGaInAsQW_{0.11}"              #Quantum Well
material_substrate = "AlGaAs_{0}"              #Substrate material
#Find graded A layer materials
n_Grading_steps = 16 #Number of grading steps to use in n side A Layer
x_max = float(material_A_max.split("{")[1].split("}")[0])
x_min = float(material_A_min.split("{")[1].split("}")[0])
material_A_vector = ["AlGaAs_{{{:.2g}}}".format(x_max-(x_max-x_min)/(n_Grading_steps-1)*i) for i in range(n_Grading_steps)]
#number of layers
N_t = 4                    #Number of Bragg bilayers on top
N_b = 6                    #Number of Bragg bilayers on bottom
N_QW = 3                   #Number of Quantum Wells
#thickness of layers
d_A_undoped = 40e-9        #A Layer (undoped section) thickness
d_A_b = 128e-9             #Bottom A Layer (doped section) thickness
d_b1_internal = 2.5e-9     #Barrier 1 thickness' (between quantum wells)
d_b1_external = 5e-9       #Barrier 1 thickness' (outside quantum wells)
d_b2 = 5e-9                #Barrier 2 thickness' (between and outside quantum wells)
d_QW = 7e-9                #Quantum Well thickness
d_blocking = 70e-9         #Blocking Layer thickness
d_A_t = 20e-9              #Top A Layer (doped section) thickness
d_cap = 100e-9             #Cap Layer thickness
...
doping_blocking = 5e23     #Blocking layer doping [m^-3]
...
## Define Structure
#Layer Names Vector
barrier_layers = ["Barrier2"]
for i in range(1,N_QW+1):
	barrier_layers = [*barrier_layers,"Barrier1","QW"+str(i),"Barrier1","Barrier2"]
layer_names = ["Substrate",                                              # Substrate
			 *["Bragg1", "Bragg2"]*N_b,                                  # Bragg stack
			 "MatchingLayer",                                            # Matching Layer
			 *["ALayer"]*len(material_A_vector), "ALayer",               # A Layer (graded, doped and undoped)
			 *barrier_layers,                                            # Barrier/QW structure
			 "ALayer", "BlockingLayer", "ALayer",                        # A Layer with Blocking layer
			 "MatchingLayer",                                            # Matching Layer
...
materials = [material_substrate,                                         # Substrate
			 ...
			 *material_A_vector, material_A_min,                         # A Layer (graded, doped and undoped)
			 *[material_barrier_2, material_barrier_1, material_qw, material_barrier_1]*N_QW, material_barrier_2,  # Barrier/QW structure
			 material_A_max, material_blocking, material_A_max,          # A Layer with Blocking layer
			 material_matching,                                          # Matching Layer
...
doping = [doping_substrate,                                              # Substrate
		  ...
		  *[doping_n_core]*n_Grading_steps, doping_deep_core,            # A Layer (graded, doped and undoped)
		  *[doping_deep_core, doping_deep_core, doping_deep_core, doping_deep_core]*N_QW, doping_deep_core,  # Barrier/QW structure
		  doping_deep_core, doping_blocking, doping_p_core,              # A Layer with Blocking layer
...
index_A_b = A_indices[0:n_Grading_steps]                                               # Bottom A Layer (doped)
...
index_b_1 = [i for i in range(len(layer_names)) if layer_names[i] == "Barrier1"]
index_b_1_external = [index_b_1[0],index_b_1[-1]]                                      # Barrier 1 External
index_b_1_internal = index_b_1[1:-1]                                                   # Barrier 1 Internal
index_b_2 = [i for i in range(len(layer_names)) if layer_names[i] == "Barrier2"]       # Barrier 2
index_QW = [i for i in range(len(layer_names)) if layer_names[i][:2] == "QW"]          # Quantum Wells
index_blocking = index_A_undoped[-1]+1                                                 # Blocking Layer
...
Nc = Jth*tau/((N_QW*d_QW+(N_QW+1)*d_b1_internal)*100*e) #Injected carriers at threshold [cm^-3]. Total thickness should be N_QW*d_QW+(N_QW+1)*(d_b1_internal+d_b2)+d_b1_external*2, but left as is to match Meng
d_n = [-1e-20*Nc if i in [*index_QW,*index_b_1_internal,*index_b_1_external,*index_b_2] else 0 for i in range(len(materials))]
...
n_p_max = min([n_p[i] if i != index_blocking else 10 for i in range(1,len(n_p)-1)]) #exclude blocking layer
num_points = 500
...
for i in range(len(n_eff_p)):
	...
	for j in index_A_b:
		d_vector[j] = d_A_b/n_Grading_steps
	for j in index_b_1_internal:
		d_vector[j] = d_b1_internal
	for j in index_b_1_external:
		d_vector[j] = d_b1_external
	for j in index_b_2:
		d_vector[j] = d_b2
	for j in index_QW:
		d_vector[j] = d_QW
	d_vector[index_blocking] = d_blocking
...
if generate_PICS3D_files:
	...
	PICS3D_file_generation(wafer_name, crosslight_materials, d_vector, doping, d_grading, index_A_b[0], index_A_t, index_QW, index_peak, etch_depth)
