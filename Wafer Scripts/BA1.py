...
# phase matched. Created to produce BA1 designed by Meng Iu.              #
#                                                                         #
# Left in the model to match Meng:                                        #
# -adjusts refractive index in QWs (not barriers) due to carrier          #
#  injection (but uses wrong thickness to calculate this)                 #
# -additional 1.1 to refractive index QWs (not barriers)                  #
#                                                                         #
...
lambda_p  = 792e-9                     #Pump (BRW) wavelength
...
material_A = "AlGaAs_{0.75}"                   #A Layer
material_barrier = "AlGaAs_{0.35}"             #Barrier
...
d_n = [-1e-20*Nc+1.1 if i in [*index_QW] else 0 for i in range(len(materials))] #additional 1.1 change to index (Meng). Only using Nc for QWs even though thickness of entire barrier/QW structure was used, but left as is to match Meng
