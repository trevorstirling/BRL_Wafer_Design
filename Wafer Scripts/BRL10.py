...
# phase matched. Created to reproduce BRL10 designed by Meng Iu.          #
#                                                                         #
...
lambda_p  = 775e-9                     #Pump (BRW) wavelength
...
material_1 = "AlGaAs_{0.2}"                    #Bragg stack material b
material_2 = "AlGaAs_{0.6}"                    #Bragg stack material a (next to core)
...
#number of layers
N_t = 5                    #Number of Bragg bilayers on top
...
d_A_b = 180e-9             #Bottom A Layer (doped section) thickness
d_b = 10e-9                #Barrier thickness' (between quantum wells)
d_QW = 4.6e-9              #Quantum Well thickness
d_A_t = 180e-9             #Top A Layer (doped section) thickness
...
Nc = Jth*tau/(N_QW*d_QW*100*e) #Injected carriers at threshold [cm^-3]
d_n = [-1e-20*Nc+1.1 if i in index_QW else 0 for i in range(len(materials))] #additional 1.1 change to QW index (Meng)
